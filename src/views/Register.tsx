...
import RegisterBox from '../items/RegisterBox'
...
const Register: React.FC = () => {
  ...
  const switchInputXPos = 1.7
  const switchInputYPos = 9.5
  const switchInputOpenPath = `
    ...
    L ${width * (switchInputXPos + 4.4*setEnableWireXScale) / resolution} ${height * (switchInputYPos - 3.5*yScale) / resolution}
  `
  const switchInputClosedPath = `
    ...
    L ${width * (switchInputXPos + 4.45*setEnableWireXScale) / resolution} ${height * (switchInputYPos - 3.5*yScale) / resolution}
  `
  ...
  const enableWireXPos = 4.3
  const enableWireYPos = 9.5
  const enableWireOffPath = `
    ...
    L ${width * (enableWireXPos + 4.4*setEnableWireXScale) / resolution} ${height * (enableWireYPos - 3.5*yScale) / resolution}
  `
  const enableWireOnPath = `
    ...
    L ${width * (enableWireXPos + 4.45*setEnableWireXScale) / resolution} ${height * (enableWireYPos - 3.5*yScale) / resolution}
  `
