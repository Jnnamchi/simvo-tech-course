...
const registerWidth = props.height / (RAM_ROW_SIZE*2)
const registerHeight = props.height / (RAM_ROW_SIZE*2)
...
function getColWirePath(colIndex: number) {
  const startX = colIndex * registerWidth + props.width * 0.5
  const startY = props.height * 0.247 // start a bit above register grid
  const downAmount = (RAM_ROW_SIZE * 6.29) + 0.3
...
function getRowWirePath(rowIndex: number) {
  const startX = props.width * 0.5 - 2
  const startY = rowIndex * registerHeight + props.height * 0.247 + 2 // start a bit above register grid
  const rightAmount = (RAM_ROW_SIZE * 3.65) + 0.3// end a bit below
...
function getRamBorderPath () {
  return new PathBuilder(props.width, props.height, props.width * 0.5 - 10, props.height * 0.236, 10)
  .right(3.08)
  .down(5.28)
  .left(3.08)
  .up(5.395)
...
function getInputDecoderPath () {
  return new PathBuilder(props.width, props.height, props.width * 0.295, props.height * 0.515, 10)
  .down(3.2)
  .build()
}
  ...
  <div>
    ...
    >
      ...
      <Wire
        ...
        :key="index"
        :pathOn="getInputMarBitPathOn(index/24 + 0.56)"
        :pathOff="getInputMarBitPathOff(index/24 + 0.56)"
        :isOn="wire.bitValue"
        :circleStart="true"
        :strokeWidth="strokeWidth"
        :circleSize="circleSize"
      ...
      <ByteWire
        ...
        :getPath="(index) => {
          ...
          const startY = 0.65*props.height + index*spacing
      ...
      <RegisterBox
        ...
        :x="(index % RAM_ROW_SIZE) * registerWidth + props.width * 0.5"
        :y="Math.floor(index / RAM_ROW_SIZE) * registerHeight + props.height * 0.25"
